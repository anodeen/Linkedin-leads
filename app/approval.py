...
class ApprovalWorkflow:
    ...
    def purge_lead(self, lead_id: int) -> int:
        to_delete = [revision_id for revision_id, item in self._items.items() if item.lead_id == lead_id]
        for revision_id in to_delete:
            del self._items[revision_id]
    ...
    def purge_older_than(self, cutoff) -> int:
        to_delete = [
            revision_id
            for revision_id, item in self._items.items()
