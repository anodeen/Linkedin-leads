...
class LeadIngestionService:
    """PRD Step 1-7: ingestion, scoring, generation, approval, delivery, dashboard, and CRM sync."""
    ...
    def sync_crm_outcome(
        ...
        if self.store.get_by_id(lead_id) is None:
            raise ValueError(f"lead_id {lead_id} not found")
        return self.crm_sync.sync_outcome(lead_id=lead_id, status=status, deal_value=deal_value)
