...
from .compliance import ComplianceReporter, ComplianceSnapshot
...
class LeadIngestionService:
    ...
    def __init__(
        ...
        compliance: ComplianceReporter | None = None,
    ) -> None:
        ...
        self.compliance = compliance or ComplianceReporter()
    ...
    def request_lead_deletion(self, lead_id: int, reason: str) -> dict[str, int]:
        if len(reason.strip()) < 3:
            raise ValueError("reason must be at least 3 characters")
        if self.store.get_by_id(lead_id) is None:
            raise ValueError(f"lead_id {lead_id} not found")

        approvals_purged = self.approvals.purge_lead(lead_id)
        delivery_events_purged = self.delivery.purge_lead(lead_id)
        crm_outcomes_purged = self.crm_sync.purge_lead(lead_id)
        lead_removed = 1 if self.store.delete_by_id(lead_id) else 0

        summary = {
            "lead_removed": lead_removed,
            "approvals_purged": approvals_purged,
            "delivery_events_purged": delivery_events_purged,
            "crm_outcomes_purged": crm_outcomes_purged,
        }
        self.audit.append(
            action="lead_deleted",
            payload={"lead_id": lead_id, "reason": reason.strip(), **summary},
        )
        return summary

    def enforce_retention_policy(self, retention_days: int) -> dict[str, int]:
        if retention_days < 1:
            raise ValueError("retention_days must be >= 1")

        cutoff = datetime.now(timezone.utc) - timedelta(days=retention_days)
        leads_purged = self.store.purge_older_than(cutoff)
        approvals_purged = self.approvals.purge_older_than(cutoff)
        delivery_purged = self.delivery.purge_older_than(cutoff)
        crm_purged = self.crm_sync.purge_older_than(cutoff)
        audit_purged = self.audit.purge_older_than(cutoff)

        summary = {
            "leads_purged": leads_purged,
            "approvals_purged": approvals_purged,
            "delivery_events_purged": delivery_purged,
            "crm_outcomes_purged": crm_purged,
            "audit_events_purged": audit_purged,
        }
        self.audit.append(
            action="retention_policy_enforced",
            payload={"retention_days": retention_days, **summary},
        )
        return summary
    ...
    def get_compliance_snapshot(self) -> ComplianceSnapshot:
        return self.compliance.build(
            leads=self.store.list_all(),
            approvals=self.approvals.list_all(),
            delivery_events=self.delivery.list_events(),
            crm_outcomes=self.crm_sync.list_outcomes(),
