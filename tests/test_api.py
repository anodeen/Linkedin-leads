...
def test_lead_deletion_purges_related_records_and_blocks_access() -> None:
    service = LeadIngestionService()
    ...
    ingest = service.ingest(
        ...
        leads=[
            InboundLead(
                ...
                title="Head of Sales",
                company="Purge SaaS",
                profile_url="https://www.linkedin.com/in/delete-me",
                source=DataSource.OFFICIAL_API,
            )
        ],
    )
    ...
    approval = service.submit_draft_for_approval(lead_id, draft)
    service.review_draft(approval.revision_id, reviewer="Manager", approve=True)
    service.send_approved_message(lead_id, recipient_email="delete.me@purge.example")
    service.record_delivery_event(lead_id, DeliveryEventType.DELIVERED, "delete.me@purge.example")
    service.sync_crm_outcome(lead_id, status=OutcomeStatus.WON, deal_value=5000)

    summary = service.request_lead_deletion(lead_id, reason="DSAR request")

    assert summary["lead_removed"] == 1
    assert summary["approvals_purged"] >= 1
    assert summary["delivery_events_purged"] >= 2
    assert summary["crm_outcomes_purged"] == 1

    assert service.list_delivery_events(lead_id=lead_id) == []
    assert service.list_crm_outcomes(lead_id=lead_id) == []

    with pytest.raises(ValueError, match="not found"):
        service.score_lead(lead_id)


def test_retention_policy_purges_old_records() -> None:
    service = LeadIngestionService()
    ...
    ingest = service.ingest(
        ...
        leads=[
            InboundLead(
                ...
                title="Head of Sales",
                company="Old SaaS",
                profile_url="https://www.linkedin.com/in/old-lead",
                source=DataSource.OFFICIAL_API,
            )
        ],
    )
    ...
    approval = service.submit_draft_for_approval(lead_id, draft)
    service.review_draft(approval.revision_id, reviewer="Manager", approve=True)
    service.send_approved_message(lead_id, recipient_email="old.lead@old.example")
    service.sync_crm_outcome(lead_id, status=OutcomeStatus.WON, deal_value=1000)

    old_time = datetime.now(timezone.utc) - timedelta(days=45)
    lead = service.store.get_by_id(lead_id)
    assert lead is not None
    lead.created_at = old_time

    for item in service.approvals.list_all():
        item.submitted_at = old_time
    for event in service.delivery.list_events():
        event.created_at = old_time
    for outcome in service.crm_sync.list_outcomes():
        outcome.captured_at = old_time
    for event in service.audit.list_events():
        event.created_at = old_time

    summary = service.enforce_retention_policy(retention_days=30)

    assert summary["leads_purged"] >= 1
    assert summary["approvals_purged"] >= 1
    assert summary["delivery_events_purged"] >= 1
    assert summary["crm_outcomes_purged"] >= 1

    assert service.list_leads() == []
    assert service.list_delivery_events() == []
    assert service.list_crm_outcomes() == []

    actions = [event.action for event in service.list_audit_events()]
    assert actions == ["retention_policy_enforced"]


def test_compliance_snapshot_rolls_up_control_metrics() -> None:
    service = LeadIngestionService()
    ...
    ingest = service.ingest(
        ...
        leads=[
            InboundLead(
                ...
                title="Head of Sales",
                company="Comp SaaS",
                profile_url="https://www.linkedin.com/in/comp-one",
                source=DataSource.OFFICIAL_API,
            )
        ],
    )
    ...
    service.submit_draft_for_approval(lead_id, draft)

    snapshot = service.get_compliance_snapshot()

    assert snapshot.total_leads == 1
    assert snapshot.pending_approvals == 1
    assert snapshot.delivery_events_total == 0
    assert snapshot.crm_outcomes_total == 0
    assert snapshot.total_audit_events >= 2

    service.request_lead_deletion(lead_id, reason="DSAR")
    service.enforce_retention_policy(retention_days=30)

    snapshot_after = service.get_compliance_snapshot()

    assert snapshot_after.dsar_deletions_recorded >= 1
    assert snapshot_after.retention_runs_recorded >= 1
